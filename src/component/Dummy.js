...
const SearchBar = () => {
    ...
    return (
        <div className='' >
            ...
            {query.length > 0 && (
                <div className='w-[100%] md:w-[70%] leading-7 h-[200px] overflow-y-scroll bg-white rounded-xl text-[#291507] font-bold 
               p-3 md:p-10 '>
                    {query.map((q, i) => (

                        <div key={q.id}>
                            <div onClick={handleStorage} className='cursor-pointer'>
                                {q.name}
                            </div>
                        </div>
                    ))}
